...
function checkGeometry() {
  ...
  for (const lanes of [1, 2, 3]) {
    const rd = new Road(lanes, 220, 760);
    assert(approx(rd.mainLw, rd.baseLw * MAIN_LANE_SCALE), `${lanes}L main width drifted from base*1.10`);
    assert(approx(rd.branchLw, rd.baseLw * BRANCH_LANE_SCALE), `${lanes}L branch width drifted from base*1.25`);
    assert(rd.preSplitInnerBoundarySampleCount === 0, `${lanes}L emitted pre-split inner samples`);
    assert(rd.splitGapAt(rd.splitWallStartT) >= SPLIT_WALL_GAP, `${lanes}L split wall starts before the configured gap`);
    ...
    for (let i = 1; i <= 100; i++) {
      ...
      assert(half + 1e-6 >= prev, `${lanes}L branch width shrank after the settle point`);
      prev = half;
      maxHalf = Math.max(maxHalf, half);
    }
    assert(maxHalf <= rd.branchHalfW('left', 1) + 1, `${lanes}L branch width overshot the steady-state width`);

    const prematureInnerSegments = rd.boundary.filter(seg =>
      seg.role === 'branch_inner' && Math.min(seg.t0, seg.t1) < rd.splitWallStartT - 1e-6
    ).length;
    assert(prematureInnerSegments === 0, `${lanes}L created inner wall segments before split start`);
    ...
    for (const sample of boundarySamples) {
      ...
      assert(Math.abs(clearance) <= 0.9, `${lanes}L boundary and legality drifted apart by ${clearance.toFixed(2)}px`);
    }
